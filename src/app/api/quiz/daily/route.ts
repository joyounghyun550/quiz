...
export async function GET() {
  ...
  const { data: tierQ } = (await supabase
    .from("questions")
    .select("*")
    .eq("is_active", true)
    .gte("difficulty", minDiff)
  ...
  const { data: session } = (await supabase
    .from("quiz_sessions")
    .insert({ user_id: user.id, session_type: "daily" as const, total_questions: selectedQuestions.length })
    .select("id")
    .single()) as { data: { id: string } | null };

  if (!todayLog) {
    await supabase.from("daily_quiz_log").insert({ user_id: user.id, quiz_date: today, session_id: session?.id });
  } else {
